...
namespace HomeWork3
{
    internal class Vector
    {
        ...
        public int[] SortArrayOfMerge(int left, int right, string path)
        {
            int[] array = ReadFromFile(path);
            if (left < right)
            {
                int middle = left + (right - left) / 2;
                SortArrayOfMerge(left, middle, path);
                SortArrayOfMerge(middle + 1, right, path);
